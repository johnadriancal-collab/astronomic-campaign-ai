...
# --- CANNOT: lifecycle transitions other than ready/unlock ------------------


def test_operator_cannot_activate(client, configured_service_operator_token):
    c, _svc = client
...
def test_normal_session_auth_for_operator_scoped_routes_is_completely_unchanged(
    ...
    actions the OPERATOR token itself cannot reach (e.g. activate)."""
    c, _svc = client
    _login(c)

    assert c.post("/mail/campaigns").status_code == 200
    assert c.post("/mail/campaigns/c1/activate").status_code == 200
