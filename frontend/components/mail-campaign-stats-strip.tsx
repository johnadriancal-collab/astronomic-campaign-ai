...
import { bounceRateDisplay, openRateDisplay } from "@/lib/mail";

// Campaign detail stats strip (2026-09-17, real Open rate 2026-09-18,
// real Bounce rate 2026-09-18) -- Open rate / Reply rate / Unsub rate /
// Bounce rate, QuickMail-density reference. Reply rate and Unsub rate
// are real, computed backend-side (MailCampaignStatsService) -- so is
// Open rate, opt-in per campaign (see lib/mail.ts's openRateDisplay(),
// the SAME helper the Campaigns list uses, so the two can never show
// conflicting copy) -- and so is Bounce rate now, automatic for every
// campaign with no toggle (see bounceRateDisplay(), also shared with
// the Campaigns list), never a fabricated 0%.
export function MailCampaignStatsStrip({ stats }: { stats: MailCampaignStats | null }) {
  const openRate = stats ? openRateDisplay(stats) : null;
  const bounceRate = stats ? bounceRateDisplay(stats) : null;
  return (
    <div className="grid grid-cols-1 divide-y divide-border rounded-lg border border-border text-sm sm:grid-cols-4 sm:divide-y-0 sm:divide-x">
      ...
      <StatItem label="Bounce rate" value={bounceRate ? bounceRate.text : "…"} tooltip={bounceRate?.tooltip ?? undefined} />
    </div>
  );
}
