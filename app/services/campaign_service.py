...
class CampaignService:
    ...
    async def preview(self, prompt: str, desired_prospect_count: int = 25) -> Campaign:
        ...
        await self.activity_log.record(
            ...
            summary=f'Campaign "{plan.campaign_name}" was created.',
            entity_type="campaign",
            entity_id=campaign.campaign_id,
            entity_name=plan.campaign_name,
        )
        return campaign
        ...
        async with self._locks[campaign_id]:
            ...
            if not campaign.apollo_list_id:
                try:
                    ...
                    await self.store.save(campaign)
                    await self._record_build_failed(campaign, msg)
                    return campaign
            ...
            await self.store.save(campaign)
            await self.activity_log.record(
                event_type="campaign.build_completed",
                category=ActivityCategory.CAMPAIGNS,
                source=ActivitySource.CAMPAIGN_SYSTEM,
                summary=f'Campaign "{campaign.plan.campaign_name}" build completed.',
                entity_type="campaign",
                entity_id=campaign.campaign_id,
                entity_name=campaign.plan.campaign_name,
                metadata={
                    "contacts_created": campaign.contacts_created,
                    "contacts_enrolled": campaign.contacts_enrolled,
                    "activated": campaign.activated,
                    "soft_errors": len(campaign.errors),
                },
            )
            return campaign

    async def _record_build_failed(self, campaign: Campaign, error: str) -> None:
        """Fires from build()'s two hard-failure early returns (list/sequence
        creation) -- never from a soft, per-contact failure that build() logs to
        campaign.errors but still finishes successfully overall (that case ends
        at campaign.build_completed above, with soft_errors > 0 in its metadata)."""
        await self.activity_log.record(
            event_type="campaign.build_failed",
            category=ActivityCategory.ERRORS,
            source=ActivitySource.CAMPAIGN_SYSTEM,
            summary=f'Campaign "{campaign.plan.campaign_name}" build failed: {error}',
            entity_type="campaign",
            entity_id=campaign.campaign_id,
            entity_name=campaign.plan.campaign_name,
    ...
    async def activate(self, campaign_id: str) -> Campaign:
        ...
        async with self._locks[campaign_id]:
            ...
            await self.activity_log.record(
                ...
                category=ActivityCategory.CAMPAIGNS,
                source=ActivitySource.CAMPAIGN_SYSTEM,
                summary=f'Campaign "{campaign.plan.campaign_name}" was activated.',
                entity_type="campaign",
                entity_id=campaign.campaign_id,
                entity_name=campaign.plan.campaign_name,
            )
    ...
    async def pause(self, campaign_id: str) -> Campaign:
        ...
        async with self._locks[campaign_id]:
            ...
            await self.activity_log.record(
                ...
                category=ActivityCategory.CAMPAIGNS,
                source=ActivitySource.CAMPAIGN_SYSTEM,
                summary=f'Campaign "{campaign.plan.campaign_name}" was paused.',
                entity_type="campaign",
                entity_id=campaign.campaign_id,
                entity_name=campaign.plan.campaign_name,
            )
